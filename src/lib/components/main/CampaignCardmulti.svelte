<script lang="ts">
    ...
    function getProgress(trader: Trader, type: CampaignType): number {
        const typeInfo = CAMPAIGN_TYPES[type] || CAMPAIGN_TYPES['TOTAL_VOLUME'];
        const value = typeInfo.getLeaderValue(trader);
        const max = Math.max(...(campaign.top_traders || []).map(t => typeInfo.getLeaderValue(t)));
...
<div class="campaign-card">
    ...
    <div class="leaderboard">
        ...
        <div class="desktop-leaderboard">
            <div class="leaderboard-columns">
                <div>#</div>
                <div>Trader</div>
                {#if campaign.campaign_type === 'HIGHEST_BUYER'}
                    <div class="text-right">Buy Volume</div>
                    <div class="text-right">Buy Count</div>
                {:else if campaign.campaign_type === 'HIGHEST_SELLER'}
            ...
            {#if filteredTraders.length}
                {#each filteredTraders.slice(0, 5) as trader, i}
                    <div class="leaderboard-row">
                        ...
                        {:else if campaign.campaign_type === 'TRADE_COUNT'}
                            <div class="value">{trader.trade_count}</div>
                            <div class="volume">${nFormatter(trader.total_volume)}</div>
                        {:else}
                            <div class="value">${nFormatter(trader.total_volume)}</div>
                            <div class="count">{trader.trade_count}</div>
                        {/if}
                        <div class="progress">
                            <div 
                                class="progress-bar" 
                                style="--progress: {getProgress(trader, campaign.campaign_type)}%">
                            </div>
                        </div>
                    </div>
                {/each}
    ...
    <div class="stats-grid">
        ...
        <div class="stats-item">
            <span class="stats-label">{typeInfo.title} Leader</span>
...
<style>
    ...
    .leaderboard-columns {
        display: grid;
        grid-template-columns: 2rem minmax(120px, 1fr) repeat(3, auto);
        gap: 1rem;
        padding: 0.5rem;
        color: var(--text-light);
        font-size: 0.75rem;
